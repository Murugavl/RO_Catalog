...
export default function ModelDetail() {
  ...
  const handleWhatsApp = () => {
    if (!product) return;
    const message = encodeURIComponent(`Hi 😊 I was checking your ${product.name} model.\nCan you share more details and installation information?`);
  ...
  const handleEmail = () => {
    if (!product) return;
    window.location.href = `mailto:${CONTACT_INFO.email}?subject=Inquiry about ${product.name}`;
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className="p-8 lg:p-12">
              ...
              <div className="grid grid-cols-2 gap-4 mb-10">
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  <span className="block text-sm text-slate-500 mb-1">Purification</span>
                  <span className="font-bold text-slate-800 text-lg">{product.purificationStages || 'N/A'}</span>
                </div>
              </div>

              {product.detailedDescription && (
                <div className="mb-10 content-formatted text-slate-600 leading-relaxed font-medium">
                  <h3 className="font-bold text-xl text-slate-800 mb-4">Detailed Description</h3>
              ...
              <div className="bg-blue-50/50 rounded-3xl p-8 border border-blue-100">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={handlePhone}
                      className="bg-blue-600 text-white py-4 rounded-xl hover:bg-blue-700 transition font-bold transform hover:-translate-y-1"
                    >
                      Call Now
                    </button>
                    <button
                      onClick={handleEmail}
                      className="bg-slate-800 text-white py-4 rounded-xl hover:bg-slate-900 transition font-bold transform hover:-translate-y-1"
                    >
                      Email Us
                    </button>
                  </div>
