...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f7ff]">
      <section
        className="py-20 md:py-32 relative overflow-hidden bg-cover bg-center"
      ...
      >
        ...
        <div className="container mx-auto px-4 text-center relative z-10 transition-all">
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold mb-6 leading-tight text-blue-900 heading-font">
            Premier RO Purifier <br className="hidden md:block" /> Solutions in Rajapalayam
          </h1>
          <p className="text-sm md:text-base lg:text-lg mb-10 max-w-xl mx-auto text-blue-800 font-medium">
            Experience the gold standard in water purification. Trusted quality for healthier homes across the region.
          </p>
          <div>
            <Link
              to="/models"
              className="inline-flex items-center justify-center bg-blue-700 text-white px-10 py-3.5 rounded-full font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-800 transition-all duration-300 transform hover:scale-105"
            >
              Explore Our Models
            </Link>
          </div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-extrabold text-center mb-12 text-slate-800">Why Choose Us?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              ...
              <div key={i} className="text-center p-8 bg-white/40 rounded-[2rem] shadow-sm border border-white/60 hover:shadow-lg transition-all duration-500">
                <div className={`${item.iconColor} w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 text-xl shadow-sm`}>
                  {i === 0 ? '🔬' : i === 1 ? '🛡️' : '👨‍🔧'}
                </div>
      ...
      {/* User Reviews Circular Carousel - Even More Compact */}
      <section className="py-12 bg-white/30 relative group">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl md:text-3xl font-extrabold mb-2 text-slate-800 tracking-tight">What People Are Saying</h2>
          <p className="text-sm text-slate-500 mb-8 font-medium italic">Trusted by thousands of families in Rajapalayam.</p>
          
          <div className="relative max-w-4xl mx-auto">
            {/* Arrows */}
            <button onClick={prevSlide} className="absolute left-0 top-[25%] -translate-y-1/2 bg-white w-8 h-8 rounded-full shadow-md flex items-center justify-center text-blue-400 hover:text-blue-600 transition-all opacity-0 group-hover:opacity-100 z-20"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M15 19l-7-7 7-7"/></svg></button>
            <button onClick={nextSlide} className="absolute right-0 top-[25%] -translate-y-1/2 bg-white w-8 h-8 rounded-full shadow-md flex items-center justify-center text-blue-400 hover:text-blue-600 transition-all opacity-0 group-hover:opacity-100 z-20"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M9 5l7 7-7 7"/></svg></button>

            {/* Circular Profiles Row - Compact w-20 and w-10 */}
            <div className="flex items-center justify-center gap-6 md:gap-8 mb-6">
              {/* Prev User */}
              <div className="hidden md:flex w-10 h-10 rounded-full bg-blue-100 items-center justify-center text-base font-bold text-blue-300 opacity-40 transform scale-90 transition-all shadow-inner">
                {reviews[getIdx(-1)].name.charAt(0)}
              </div>
              
              {/* Current User */}
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-blue-600/10 relative p-1 shadow-md transition-all duration-500 transform hover:scale-105">
                <div className="w-full h-full rounded-full bg-blue-900 flex items-center justify-center text-2xl md:text-3xl font-serif font-black text-white border-2 border-white/10">
